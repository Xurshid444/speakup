...
function switchTab(name){
  ...
  if(name==='vocab'){
    ...
    // Barcha sub-view larni yashirib faqat units ko'rsatish
    ['vv-lessons','vv-test','vv-score'].forEach(id=>{const el=$(id);if(el)el.style.display='none';});
    const vu=$('vv-units'); if(vu) vu.style.display='block';
  }
  if(name==='spelling'){
    const g=$('sunit-grid');
    if(g && g.children.length===0) renderSpellUnits();
    // Barcha sub-view larni yashirib faqat units ko'rsatish
    ['sv-lessons','sv-game','sv-score'].forEach(id=>{const el=$(id);if(el)el.style.display='none';});
    const su=$('sv-units'); if(su) su.style.display='block';
  }
  if(name==='mydict'){
